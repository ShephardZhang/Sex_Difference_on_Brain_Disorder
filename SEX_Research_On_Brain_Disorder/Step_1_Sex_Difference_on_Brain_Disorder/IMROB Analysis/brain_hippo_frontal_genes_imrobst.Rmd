...
brain_hippo_tmp=read.table("/Users/muyangzhang/Desktop/research_sex/gtxforr/tpm_tissue/gene_tpm_brain_hippocampus.gct",stringsAsFactors = F,header=T,skip=2,check.names = F,sep='\t')

brain_hippo_tmp_clean=brain_hippo_tmp[,c(2,4:dim(brain_hippo_tmp)[2])]
...
brain_hippo_tmp_new=as.data.frame(t(brain_hippo_tmp_clean))
names(brain_hippo_tmp_new)=brain_hippo_tmp_new[1,]
brain_hippo_tmp_new=brain_hippo_tmp_new[-1,]
brain_hippo_tmp_new=cbind(data.frame(Tissue.ID=row.names(brain_hippo_tmp_new),
                               Individual.ID=unlist(sapply(row.names(brain_hippo_tmp_new),function(x) sub("^([^\\-]+\\-[^\\-]+).*", "\\1", x)))),brain_hippo_tmp_new)
head(brain_hippo_tmp_new)
```
...
brain_hippo_cov=read.table("/Users/muyangzhang/Desktop/research_sex/gtxforr/cis_qtl/GTEx_Analysis_v8_eQTL_covariates/Brain_Hippocampus.v8.covariates.txt",stringsAsFactors = F,header=T,check.names = F,sep='\t')

brain_hippo_cov_new=as.data.frame(t(brain_hippo_cov))
names(brain_hippo_cov_new)=brain_hippo_cov_new[1,]
brain_hippo_cov_new=brain_hippo_cov_new[-1,]
brain_hippo_cov_new=cbind(data.frame(Individual.ID=row.names(brain_hippo_cov_new)),brain_hippo_cov_new)
...
brain_hippo_tmp_pheno=merge(brain_hippo_tmp_new,phenotype,by.x='Individual.ID',by.y='SUBJID')
brain_hippo_tmp_pheno_cov=merge(brain_hippo_tmp_pheno,brain_hippo_cov_new,by='Individual.ID')
...
values_to_replace <- unique(brain_hippo_tmp_pheno_cov$AGE)
```
...
brain_hippo_tmp_pheno_cov$AGE = sapply(brain_hippo_tmp_pheno_cov$AGE, function(x)
                                 replacement_values[which(x==values_to_replace)])
head(brain_hippo_tmp_pheno_cov)
```
...
gene_col_ID=which(grepl("ENSG",names(brain_hippo_tmp_pheno_cov)))

gene_mean=data.frame(geneID=names(brain_hippo_tmp_pheno_cov)[gene_col_ID],
                     mean_TPM=apply(brain_hippo_tmp_pheno_cov[,gene_col_ID],2,function(x) mean(as.numeric(x))))
...
```
> There are 39939 genes with TPM < 1, which can be filtered during the analysis.
...
brain_hippo_tmp_pheno_cov_TPM_filtered=brain_hippo_tmp_pheno_cov[,!names(brain_hippo_tmp_pheno_cov) %in%gene_ID_failed]
dim(brain_hippo_tmp_pheno_cov_TPM_filtered)
```
...
Covariate_header=c(names(brain_hippo_tmp_pheno_cov_TPM_filtered)[grepl('Inferred',names(brain_hippo_tmp_pheno_cov_TPM_filtered))],"PC1","PC2","PC3","PC4","PC5")

lm_sex <- function(Expression_header) {
  # Function body
  # Perform operations using arguments and other statements
  block=brain_hippo_tmp_pheno_cov_TPM_filtered[,c(Expression_header,Phenotype_header,Covariate_header)]
...
# Extract gene list
gene_list <- names(brain_hippo_tmp_pheno_cov_TPM_filtered)[grepl("ENSG", names(brain_hippo_tmp_pheno_cov_TPM_filtered))]
...
ggplot(brain_hippo_tmp_pheno_cov_TPM_filtered, aes(x = SEX, y = RankNorm(as.numeric(ENSG00000157870.14)), fill = as.factor(SEX))) +
  geom_boxplot() +
...
write.table(results, "brain_hippo_lmrob_SEX_multhread_with_ranknorm_FDR.txt", quote=F, row.names = F, sep='\t')
